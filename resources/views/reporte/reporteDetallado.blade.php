	...
	<div class="col-md-12">
		<div class="row">
			<div class="col-md-8">
              ...
              <label for="selectproyectos" >{{ trans('message.nameproye')}}</label>
              <div class="selectContainer">
                	<select id="clienteProyectos" name="clienteProyectos"  class="form-control select2" onchange="ocultarDa(); validarSelectreg('clienteProyectos');" style="width: 200px" required>
                        ...
                        @foreach ($selectproyect as $info)
                        <option value="{{ $info->Nombre_proyecto }}"> {{ $info->Nombre_proyecto }} </option>
                        @endforeach
        				   </select>
					 ...
					 <div class="form-group">
              <label for="selectregprecios" >{{ trans('message.cliente')}}</label>
             	<div class="selectContainer">
      ...
      <div id="titulos" name="titulos" class="row">
        <div class="col-xs-12">
          <h2 class="page-header">
            <i class="fa fa-desktop"></i> {{ trans('message.reportdethotel') }}
	        ...
	        <div class="col-xs-9">
		            ...
		            <table class="table table-bordered">
		            	<thead>
						    <tr>
						      ...
						      <th class="bg-primary">{{ trans('message.stock') }} </th>
						      <th class="bg-primary">{{ trans('message.prestamo') }} </th>
						      <th class="bg-primary">{{ trans('message.venta') }} </th>
						      <th class="bg-primary">{{ trans('message.renta') }} </th>
						      <th class="bg-primary">{{ trans('message.demo') }} </th>
						      <th class="bg-primary">{{ trans('message.cambio') }} </th>
						      <th class="bg-primary">{{ trans('message.garantia') }} </th>
						    </tr>
						</thead>
						<tbody>
							<tr>
		        ...
		        <div class="table-responsive" style="padding-top: 30px;">
		            <table class="table table-bordered">
		            	<thead >
        					    <tr>
        					    	<th colspan="3" class="bg-primary"> {{ trans('message.estadoswitches') }}</th>
        	              <th colspan="3" class="bg-primary"> {{ trans('message.estadosdirector')}}</th>
        					    </tr>
        					</thead>
        					<tbody>
          						<tr>
  			            		<td>{{ trans('message.prestamo') }}</td>
  			            		<td>{{ trans('message.demo') }}</td>
  			            		<td>{{ trans('message.renta') }}</td>
  			            		<td>{{ trans('message.venta') }}</td>
  			            		<td>{{ trans('message.renta') }}</td>
  			            		<td>{{ trans('message.demo') }}</td>
  			            	</tr>
        							<tr>
	        ...
	        <div class="col-xs-12 table-responsive">
	          <table class="table table-striped table-bordered" id="contentTableDetalles" name="contentTableDetalles">
	            ...
	            <tr>
	              ...
	              <th>{{trans('message.marca')}}</th>
	              <th>{{trans('message.estad')}}</th>
	              <th>{{trans('message.nameserv')}}</th>
	            </tr>
	            </thead>
